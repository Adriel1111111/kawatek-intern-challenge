...
function App() {
  ...
  const enrichedMetrics = useMemo(() => metrics.map((metric, index) => {
    const iconMap = [FiActivity, FiTrendingUp, FiHeart, FiClock]
    const trendValue = index === 0
      ? 'Live dataset'
      : index === 1
        ? `▲ ${Math.abs(progressImprovement)}%`
        : index === 2
          ? `▲ ${Math.round(((averageEmg || 0) - 0.75) * 100)} pts`
          : `▲ ${totalTherapyDuration} min`
  ...
  }), [averageEmg, metrics, progressImprovement, totalTherapyDuration])

  const hasData = Boolean(patient?.name) && sessions.length > 0

  const handleExportSummary = async () => {
    setIsExporting(true)

    try {
      const blob = await pdf(<PdfExportDocument
        patient={patient}
        metrics={metrics}
        summary={{ ...summary, score: recoveryScore }}
        recommendations={recommendations}
        sessions={sessions}
        latestSession={latestSession}
        averageProgress={averageProgress}
        averageEmg={averageEmg}
        totalTherapyDuration={totalTherapyDuration}
      />).toBlob()

      const url = URL.createObjectURL(blob)
      const link = document.createElement('a')
      link.href = url
      link.download = 'activai-rehabilitation-summary.pdf'
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_#f8fbff,_#f4f7fb_55%,_#eef4fb)] px-4 py-5 text-slate-800 sm:px-6 lg:px-8 lg:py-8" aria-label="Rehabilitation dashboard">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <Header patient={patient} summary={headerSummary} onExport={handleExportSummary} isExporting={isExporting} />

        {!hasData ? (
          <PanelCard title="Data availability" subtitle="The dashboard is ready, but the latest session data is unavailable" action="No data">
            <EmptyState title="No rehabilitation data available" message="The current dataset appears incomplete or unavailable. Once the latest patient sessions are loaded, the dashboard will populate automatically." />
          </PanelCard>
        ) : (
          <>
            ...
            <PanelCard title="ACTIVAI Clinical Intelligence" subtitle="Data-driven clinical guidance generated from the current patient dataset" action="AI-ready">
              <ClinicalInsightsPanel summary={{ ...summary, score: recoveryScore }} warnings={warnings} recommendations={recommendations} />
            </PanelCard>
